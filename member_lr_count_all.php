...
<?php
//member details  
$mem = "select client_id,user_id,usr_name from client_list where client_id = '113'";
...
while($mem2 = mysql_fetch_array($mem1))
{
	...
	{ 
	  ...
	  //get last used pair
	  $get_lst_pair = "select lft_pt,rght_pt,cmn_pair from last_pp_detls where membr_id = '".$mem2['client_id']."'";
	  $get_lst_pair1 = mysql_query($get_lst_pair) or die(mysql_error());
	  while($get_lst_pair2 = mysql_fetch_array($get_lst_pair1))
	  {
	     $get_lst_used_pairs+=$get_lst_pair2['cmn_pair'];
	  }
